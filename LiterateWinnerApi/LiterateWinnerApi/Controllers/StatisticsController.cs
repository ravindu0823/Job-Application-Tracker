...
) : ControllerBase
{
    ...
    public async Task<IActionResult> GetStatusDistribution()
    {
        try
        {
            ...
            return Ok(ApiResponse<List<StatusDistributionDto>>.Success(distribution, "Status distribution retrieved successfully"));
        }
        catch (Exception ex)
        {
    ...
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(typeof(ApiResponse<object>), StatusCodes.Status500InternalServerError)]
    public async Task<IActionResult> GetTimelineData([FromQuery] DateTime? startDate = null, [FromQuery] DateTime? endDate = null)
    {
        try
        {
            var userId = _identityService.GetUserIdentity();
